...
class MockMailService implements MailService {
  ...
  @override
  List<MailItem> get pageItems => _pageItems;
  ...
  Future _generateItems(String label, int newPageIndex) async {
    if (selectedFolder != label) {
      ...
      _mailCount = 11 + (label.hashCode.abs() % 13) * 7;
      _pageIndex = 0;
      _pageCount = (_mailCount / pageSize).ceil();
    } else if (newPageIndex < 0 || newPageIndex >= _pageCount) {
      // invalid state, no change in the items
      return;
    } else {
  ...
  MailItem _generateItem(int index) {
    int idx = (_selectedFolder.hashCode.abs() % 197) +
        (_pageIndex * pageSize) +
        index;
    String name = _senders[idx % _senders.length];
    String email = _emails[idx % _emails.length];
    String subject = _subjects[idx % _subjects.length];
    List<String> fragments = new List.generate(
        10, (int fidx) => _fragments[(idx + fidx) % _fragments.length]);
    return new MailItem(name, email, subject, fragments.join('\n'));
  }
}
